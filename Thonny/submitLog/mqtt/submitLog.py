""""
                        Room Monitoring Using Raspberry pi pico + DHT sensor with log submission (mqtt)
        ...
        if utime.ticks_ms() - interval_time > 5000:  # interval
            ...
            if virtual_sensor:
                ...
                try:
                    ...
                    anedya_submitLog("", f"Failed to read from DHT!, Error: {str(e)}", "ERROR")
...
def anedya_submitLog(param_req_id: str, param_log: str, param_logType: str = "INFO"):
    global counter
    counter += 1
    PUBLISH_TOPIC = f"$anedya/device/{PHYSICAL_DEVICE_ID}/logs/submitLogs/json"
    ...
    log_payload = json.dumps(
        {
            "reqId": param_req_id,
            "data": [{"timestamp": current_time_ms, "log": log_with_counter}],
        }
    )
    print(f"Sending log: {log_with_counter}")
    mqtt_client.publish(PUBLISH_TOPIC, log_payload, qos=0)  # Publish humidity payload
    time.sleep(1)
    mqtt_client.check_msg()  # Check for incoming messages
